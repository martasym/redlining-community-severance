# The following code assigns a holc grade to each block group and then begins to explore
# the correlation between redlined hoc grade and community severance index.
...
# C) Second, we have to pull the San Francisco block groups from 2019, which should line up with the Community Severance Data set
  # the block groups come from the U.S. Census (last updated according to website 2019) https://catalog.data.gov/dataset/tiger-line-shapefile-2019-state-california-current-block-group-state-based
...
# Then, the block groups with the community severance index.
...
# 4) Now, we know that those block groups that <30% redlined area are not in census2019_sf_bg_holc_overlap. 
    # We will now label these in the index_join (community severance index)
        # We will give those not found within the census2019_sf_bg_holc_overlap data set a not lined designation.
index_join$HOLC_grade <- NA
...
# 5) Then, for each block group, if one grade makes up over 30% of the block group, 
  ...
  # This is done through the following for loop:
    ...
    # If one grade does not take up more than 30 percent, it is assigned the grade "tied?"
...
#   otherwise, it assigns the value "tied?" 
# Note: 04/12/26 The tied values have been removed. There were only four of them. They were all surrounded.
#   Note: 04/26/26 The tied values have been added back
...
# The following map can test the different block groups (change row).
row <- 10
red_sfblock_groups_a <- ggplot() +
  geom_sf(data = redlining_dta_sf, aes(fill = holc_grade)) +
  #geom_sf(data = faf5_highways_loc, color = "blue", fill = NA) + 
  scale_fill_manual(values=c("darkgreen", "lightblue","lightyellow", "darkred")) + 
  geom_sf(data = census2019_sf_bg_holc_overlap[row,], color = "yellow", fill=NA)
#scale_fill_viridis_c(option = "magma", direction = -1)
red_sfblock_groups_a


# 6) The holc grades for those block groups found in the census2019_sf_bg_holc_overlap data set
...
# This map contains the original redlining data, and outlines the block groups 
    # with their assigned designation by outlined color-- grade A = green, grade B = blue,
      # grade C = orange, grade D = red, and tried = outlined in purple/ filled in pink.
...
rm(grade_trial)
rm(redlining_dta_sf, redlining_sf_comp)
